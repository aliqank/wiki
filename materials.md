...
| **Схема БД v3 сводная (24.04, актуальная)** | `results/2026-04-24 - Схема БД v3 (Equipment + Booking).md` — **актуальная**: закрыт OQ-DB-9 (транспортировка unwheeled); Bookings расширена: поле `transportBookingId` (self-ref FK) + статус `TransportConfirmed`; previousSnapshot покрывает транспортный откат; добавлен OQ-DB-11 (скоуп транспортной брони) |
