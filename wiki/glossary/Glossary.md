...
| Booking Extension Approval | Отдельный approval step на изменение `plannedEndDateTime` брони по запросу Requestor / SWP. Используется, когда изменение срока требует решения Fleet Owner и не должно смешиваться с базовым FO approval при первоначальном согласовании брони. |
