...
| Параметр | Значение |
|---|---|
| Описание | Получить список приоритетов заявки для справочника Admin Panel |
...
Новый метод. Используется для экрана Admin Panel, где администратор просматривает фиксированный справочник приоритетов `P1`-`P4` и редактирует для них `description` и `color`.
...
| 4 | Локализованное описание | description | object | object | — | ref_request_priority | Редактируется в Admin Panel |
| 5 | Цвет отображения | color | string | `#RRGGBB` | — | ref_request_priority.color | Редактируется в Admin Panel |
...
2. Поля `code`, `name`, `sortOrder` в Admin UI отображаются как read-only.
