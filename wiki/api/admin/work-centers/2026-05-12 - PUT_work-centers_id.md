...
3. Провалидировать поле `name`: непустая строка; уникальная в `WorkCenters` WHERE `id` ≠ `:id` AND `isDeleted = false`.
...
| 3 | Наименование work center | `name` | `string` | `+` | Непустая строка; уникальная среди `WorkCenters` WHERE `id` ≠ `:id` AND `isDeleted = false` | — | Request body | |
...
1. Уникальность `code` и `name` при обновлении проверяется с исключением самой редактируемой записи (`id ≠ :id`).
