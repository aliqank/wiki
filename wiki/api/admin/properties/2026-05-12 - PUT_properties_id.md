...
2. Провалидировать `name`: непустая строка; уникальная среди `Properties` WHERE `id` != `:id` AND `isDeleted = false`.
...
| 2 | Наименование характеристики | `name` | `string` | `+` | Непустая строка; уникальная среди активных записей кроме текущей | — | Request body | |
