...
3. Проверить новую технику: тот же тип / допустимый бизнес-контекст, отсутствие hard-ограничений доступности на период, не `OnDemand`.
   Пересечения с другими активными бронями должны быть доступны Fleet Owner как conflict context и не блокируют замену автоматически.
...
| `EQUIPMENT_NOT_AVAILABLE` | Новая техника недоступна по hard-ограничениям; competing bookings сами по себе не вызывают эту ошибку |
