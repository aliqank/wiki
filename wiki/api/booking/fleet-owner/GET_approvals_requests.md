...
| Параметр | Значение |
|---|---|
| Описание | Получить список заявок [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md) с вложенными бронями для работы |
...
Новый метод. Используется для [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md) view `Requests` как request-centric список заявок с составом броней, достаточным для первичной обработки без обязательного дополнительного detail-запроса.
...
6. Для каждой заявки вернуть только связанные booking item-ы, которые относятся к зоне ответственности текущего [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md).
...
|---|---|
| [`FleetOwner`](../../../requirements/Roles%20and%20Access%20Model.md) | Просмотр заявок и связанных броней по управляемым флотам |
...
Метод возвращает только активные для [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md) заявки со статусами `Submitted` и `InProgress`. `Draft` и `Closed` не должны попадать в выдачу текущего списка; завершенные и архивные сценарии должны обслуживаться отдельным history flow.
...
| 10 | Количество броней, доступных текущему [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md) | bookingsCount | int | integer | — | backend aggregation | Считаются только item-ы в зоне ответственности текущего FO |
...
| 12 | Список [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md)s | fleetOwners | array<object> | object[] | `[]` | Fleets + FleetManagePermissions + Users | Только owner-assignment'ы для флота |
...
2. В `bookingSummaries` не должны попадать чужие booking item-ы, не относящиеся к зоне ответственности текущего [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md).
3. Если в заявке есть брони по нескольким fleet-ам, [`Fleet Owner`](../../../requirements/Roles%20and%20Access%20Model.md) видит только те item-ы, по которым он вправе принимать решение.
