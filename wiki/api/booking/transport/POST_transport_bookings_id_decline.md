...
4. Не переводить основную бронь в terminal status; оставить ее в `Submitted` и зафиксировать отрицательное транспортное решение как `TransportationApproval / Declined`.
5. Если transport linkage уже была создана в `BookingTransportations`, сохранить ее без изменений; текущий отказ транспортной роли не удаляет саму связь автоматически.
6. Если передана `proposedStartDateTime`, сохранить ее в комментарии/metadata до финального отдельного проектного решения.
7. При необходимости создать запись в `BookingStatuses` только если lifecycle status по итогам процесса меняется.
8. Вернуть результат.
...
- изменяются: [`BookingApprovals`](../../../db/2026-05-21%20-%20DB%20Schema%20v12%20%28Azure%20SQL%2C%20Equipments%2C%20Booking%29.md#23-bookingapprovals), [`BookingStatuses`](../../../db/2026-05-21%20-%20DB%20Schema%20v12%20%28Azure%20SQL%2C%20Equipments%2C%20Booking%29.md#24-bookingstatuses)
