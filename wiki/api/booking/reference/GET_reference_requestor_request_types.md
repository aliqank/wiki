...
Новый reference API для `UC-REQ-01`. Используется для загрузки списка значений фильтра `type` на странице `Мои заявки`.
...
|---|---|---|---|---|---|
| TCO Booking Tool | FR-091 | Requestor/SWP can search and filter own requests | Confirmed | BRD v13 | Значения фильтра `type` должны загружаться отдельным reference API |
