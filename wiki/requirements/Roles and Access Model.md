...
| `FleetOwners' Supervisor` | Финальный согласующий для `Long-term rented` после решения FO | Только queue и detail view long-term rented броней, дошедших до шага `Confirmed by FO` | Dedicated AAD-managed role | Получить уведомление, просмотреть justification, финально подтвердить бронь или отклонить ее с обязательным комментарием |
