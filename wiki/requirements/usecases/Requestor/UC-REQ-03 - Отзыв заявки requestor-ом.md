# UC-REQ-03 - Отзыв заявки requestor-ом
...
| Триггер | Нажатие кнопки `Отозвать заявку` / `Отменить draft` |
...
3. Система показывает действие `Отозвать заявку` / `Отменить draft` только для доступной собственной draft-заявки.
...
8. Backend переводит заявку в terminal status `Cancelled` и фиксирует изменение статуса в истории.
...
3. API-спецификация использует terminal status `Cancelled` для заявки, хотя базовый BRD в явном виде детализирует в основном активные request statuses.
