...
| Ожидаемый результат | Запрос на изменение `plannedEndDateTime` принят системой; до `InProgress` бронь переходит/остается в `Submitted` и ожидает решения Fleet Owner; в `InProgress` бронь сохраняет статус `InProgress`, а изменение применяется только после отдельного FO approval |
...
9. Frontend обновляет карточку и показывает актуальное состояние брони после решения Fleet Owner.
...
3. По новой дате окончания есть [booking conflict context](../../../glossary/Glossary.md#booking-conflict-context).
   Запрос не должен отклоняться автоматически только из-за overlap по `Bookings`; решение принимает Fleet Owner.

4. Пользователь пытается изменить не свою бронь.
   Backend возвращает `FORBIDDEN`.

5. Fleet Owner отклоняет запрос на изменение `plannedEndDateTime`.
