...
| Ожидаемый результат | Запрос на изменение `plannedEndDateTime` принят системой; до `InProgress` бронь переходит/остается в `Submitted` и ожидает решения [`Fleet Owner`](../../Roles and Access Model.md); в `InProgress` бронь сохраняет статус `InProgress`, а изменение применяется только после отдельного FO approval |
...
5. Если текущий статус брони равен `Submitted` или `Confirmed`, backend:
   ...
   - инициирует повторное рассмотрение со стороны [`Fleet Owner`](../../Roles and Access Model.md).
6. Если текущий статус брони равен `InProgress`, backend:
   ...
   - не применяет новое `plannedEndDateTime` до решения [`Fleet Owner`](../../Roles and Access Model.md).
7. В обоих сценариях система уведомляет [`Fleet Owner`](../../Roles and Access Model.md) о запросе на изменение срока брони.
8. После положительного решения [`Fleet Owner`](../../Roles and Access Model.md):
...
9. Frontend обновляет карточку и показывает актуальное состояние брони после решения [`Fleet Owner`](../../Roles and Access Model.md).
...
3. По новой дате окончания есть [booking conflict context](../../../glossary/Glossary.md#booking-conflict-context).
   Запрос не должен отклоняться автоматически только из-за overlap по `Bookings`; решение принимает [`Fleet Owner`](../../Roles and Access Model.md).

4. Пользователь пытается изменить не свою бронь.
   Backend возвращает `FORBIDDEN`.

5. [`Fleet Owner`](../../Roles and Access Model.md) отклоняет запрос на изменение `plannedEndDateTime`.
...
2. Для статусов `Submitted` и `Confirmed` изменение `plannedEndDateTime` означает повторное ожидание решения [`Fleet Owner`](../../Roles and Access Model.md), поэтому бронь должна находиться в `Submitted` до принятия решения.
...
4. Для `InProgress` сценария требуется отдельный [Booking Extension Approval](../../../glossary/Glossary.md#booking-extension-approval); до решения [`Fleet Owner`](../../Roles and Access Model.md) новое `plannedEndDateTime` не должно считаться примененным.
5. После положительного решения [`Fleet Owner`](../../Roles and Access Model.md) бронь должна иметь статус `Confirmed` для pre-start сценария и сохранять статус `InProgress` для сценария во время исполнения.
