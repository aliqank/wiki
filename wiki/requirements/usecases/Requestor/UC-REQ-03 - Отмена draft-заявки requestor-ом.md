...
8. Backend переводит заявку в terminal status `Cancelled`, переводит связанные draft booking item-ы в `Cancelled` и фиксирует изменения в истории.
...
3. При отмене draft-заявки связанные draft booking item-ы также переводятся в `Cancelled`, что позволяет отличать отмененные draft item-ы от живых draft item-ов.
