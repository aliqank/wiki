# UC-FO-02 - Просмотр load summary [`Fleet Owner`](../../Roles%20and%20Access%20Model.md)
...
| Поле | Значение |
|---|---|
| Область действия | Страница [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) `Approvals` -> view `Requests` -> booking row action `Load summary`; optional detail / action view конкретной брони |
| Участник | Пользователь с ролью [`FleetOwner`](../../Roles%20and%20Access%20Model.md) |
| Покрываемые FR (BRD) | `FR-NEW-64` |
| Покрываемые FR (Additional list) | — |
| Предусловие | Пользователь авторизован в системе; пользователь имеет роль [`FleetOwner`](../../Roles%20and%20Access%20Model.md); бронь относится к зоне ответственности пользователя; пользователь находится на странице `Approvals` во view `Requests` |
...
5. Backend проверяет, что:
   - бронь существует;
   - текущий пользователь имеет доступ к брони как [`Fleet Owner`](../../Roles%20and%20Access%20Model.md);
...
1. Load summary используется как информационный блок для принятия решения [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) и сам по себе не является hard-stop механизмом.
