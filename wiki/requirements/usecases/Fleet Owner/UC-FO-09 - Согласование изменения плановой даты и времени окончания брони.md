...
| Поле | Значение |
|---|---|
| Область действия | Страница [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) `Approvals` -> detail / action view брони с pending extension request |
...
| Триггер | [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) открывает бронь, по которой [`Requestor`](../../Roles%20and%20Access%20Model.md) запросил изменение `plannedEndDateTime` |
| Ожидаемый результат | [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) принимает решение по запросу на изменение срока брони; после approve новое `plannedEndDateTime` применяется, а статус брони становится `Confirmed`, если других pending approvals не требуется, либо сохраняет `InProgress`, если исполнение уже началось |
...
1. [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) открывает страницу `Approvals` и переходит в detail / action view брони, по которой есть запрос на изменение `plannedEndDateTime`.
...
3. Система показывает:
   ...
   - комментарий [`Requestor`](../../Roles%20and%20Access%20Model.md);
   - [booking conflict context](../../../glossary/Glossary.md#booking-conflict-context), если он есть.
4. [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) анализирует запрос и принимает решение.
...
6. Backend проверяет, что:
   ...
   - текущий [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) имеет право принять решение.
...
1. [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) отклоняет запрос на изменение `plannedEndDateTime`.
...
4. Данный use case описывает только решение [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) по уже существующему запросу [`Requestor`](../../Roles%20and%20Access%20Model.md) на изменение срока брони.
