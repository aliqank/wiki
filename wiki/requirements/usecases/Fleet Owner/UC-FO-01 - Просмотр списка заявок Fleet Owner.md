# UC-FO-01 - Просмотр списка заявок [`Fleet Owner`](../../Roles%20and%20Access%20Model.md)
...
| Поле | Значение |
|---|---|
| Область действия | Страница [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) `Approvals` -> view `Requests` |
...
| Предусловие | Пользователь авторизован в системе; пользователь имеет роль [`FleetOwner`](../../Roles%20and%20Access%20Model.md); пользователю доступны один или несколько fleet-ов |
| Триггер | Вход на страницу [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) `Approvals` c активной view `Requests` |
| Ожидаемый результат | Отображается paginated список заявок, относящихся к зоне ответственности [`Fleet Owner`](../../Roles%20and%20Access%20Model.md), с request-level статусом и базовыми фильтрами |
...
1. Пользователь открывает страницу [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) `Approvals` и переключается во view `Requests`.
...
4. Backend проверяет, что текущий пользователь имеет роль [`FleetOwner`](../../Roles%20and%20Access%20Model.md).
...
10. Для каждой строки отображаются request-level поля и краткий состав связанных броней, доступных текущему [`Fleet Owner`](../../Roles%20and%20Access%20Model.md):
   - номер заявки;
   - текущий статус заявки;
   - тип заявки;
   - приоритет с цветом, соответствующим настройке справочника.
11. Для каждой релевантной брони система также может показывать conflict indicator и quick action для открытия [`load summary`](UC-FO-02%20-%20Просмотр%20load%20summary%20Fleet%20Owner.md) прямо в строке брони.
...
1. У [`Fleet Owner`](../../Roles%20and%20Access%20Model.md) нет заявок в зоне ответственности.
...
2. В список не должны попадать заявки, не имеющие booking item-ов по fleet-ам текущего [`Fleet Owner`](../../Roles%20and%20Access%20Model.md).
