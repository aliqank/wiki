...
**Найденные дефекты:**
1. **TC-002, TC-010 (Work Centers); TC-019 TC-002 (Measurement Units); TC-020 TC-002 (Maintenance Partners)** — дублирующийся `code` при CREATE/UPDATE вызывает HTTP 500 `UNHANDLED`; уникальность не обрабатывается, сервер падает
2. **TC-003 (Work Centers); TC-021 TC-002 (Business Partners); TC-022 TC-002 (Properties)** — дублирующиеся значения при CREATE принимаются с HTTP 200; уникальность не проверяется
3. **TC-004, TC-011 (Work Centers); TC-021 TC-005 (Business Partners)** — отсутствие обязательного поля при CREATE/UPDATE вызывает HTTP 500 `UNHANDLED` вместо 422
4. **TC-005, TC-006, TC-007 (Work Centers); TC-020 TC-005 (Maintenance Partners); TC-022 TC-005 (Properties)** — валидация обязательных и обязательных локализованных полей отсутствует; whitespace-only значения принимаются; сервер возвращает HTTP 200
5. **TC-014, TC-015 (Work Centers)** — DELETE возвращает HTTP 200 при удалении несуществующей и уже удалённой записи; проверка существования отсутствует
6. **TC-017** — RBAC не проверяется: пользователь без роли Admin выполняет POST, PUT, DELETE с HTTP 200; подтверждён на Work Centers, аналогично FR-005 и FR-009
7. **TC-001, TC-012 (все справочники)** — HTTP статусы не соответствуют спецификации: 200 вместо 201 при создании, 200 вместо 204 при удалении
8. **Все справочники** — фактические endpoints расходятся со спекой: `api/v1/*` (ед. ч.) вместо `api/admin/v1/*` (мн. ч.); статусы 400/500 не задокументированы в Swagger
