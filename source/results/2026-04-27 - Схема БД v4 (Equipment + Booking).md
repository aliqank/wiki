# Схема БД v4: Equipment + Booking (сводный файл)
...
| # | Изменение |
|---|-----------|
| 1 | Закрыт OQ-DB-11: транспортная бронь создаётся в рамках того же `requestId`, что и unwheeled-бронь |
| 2 | EquipmentTypeProperties: составной PK заменён на `id UUID PK` + уникальный индекс |
| 3 | EquipmentProperties: составной PK заменён на `id UUID PK` + уникальный индекс |
| 4 | EquipmentMaintenanceContracts: составной PK заменён на `id UUID PK` + уникальный индекс |
| 5 | Bookings: уточнено описание `transportBookingId` — транспортная бронь обязана быть в том же `requestId` |
| 6 | Добавлен OQ-DB-12: нужно ли поле `transportRequestedFleetOwnerId` для фиксации FO-коллеги при запросе транспорта? |
...
### Новые решения (2026-04-27, рассуждения по заявке на транспортировку — OQ-DB-11)

| Тема | Решение | Обоснование |
|------|---------|-------------|
| UUID PK во всех таблицах | `id UUID PK` — отдельное поле в каждой таблице | Единое правило; составные PK переведены в UNIQUE-индексы |
| Скоуп транспортной брони (OQ-DB-11) | Транспортная бронь создаётся **в рамках того же `requestId`**, что и unwheeled-бронь | Транспортная техника обслуживает брони внутри той же заявки; заявка остаётся открытой от имени Requestor-а |
| Атрибуция FO в BookingRequests | **Не добавляем** след FO в шапку заявки | Заявка остаётся от имени Requestor-а; след FO фиксируется на уровне транспортной брони |
...
Для unwheeled-техники: транспортная бронь создаётся FO в рамках **той же заявки** (тот же `id`). Заявка остаётся открытой от имени Requestor-а — атрибуция FO фиксируется только на уровне транспортной брони.
...
Для техники с `EquipmentTypes.requiresTransport = TRUE` — после подтверждения брони FO обязан привязать транспортную бронь через `transportBookingId`. Транспортная бронь создаётся в рамках **того же `requestId`**. Статус переходит в `TransportConfirmed` как только транспорт выбран и привязан.
...
| **transportBookingId** | **UUID nullable FK → Bookings** | **Self-ref: ID брони транспортной техники, которая везёт данную единицу. Только для unwheeled-техники (requiresTransport = TRUE). Транспортная бронь обязана принадлежать тому же requestId. NULL до момента привязки транспорта FO-ом.** |
...
| 8 | EquipmentTypeProperties | Схема свойств по типу (Admin Panel); UUID PK + UNIQUE (typeId, propId) |
| 9 | EquipmentProperties | Значения свойств конкретной техники; UUID PK + UNIQUE (equipmentId, propertyId) |
...
| 14 | EquipmentMaintenanceContracts | Связь техники и партнёра ТО; UUID PK + UNIQUE (equipmentId, partnerId, serviceType) |
...
| 2 | Bookings | Атомарная единица бронирования (одна техника); self-ref FK `transportBookingId` для unwheeled; транспортная бронь — в том же requestId |
...
**Итого: 20 таблиц** (количество не изменилось)
...
| **Bookings.transportBookingId** | **Bookings.id** | **Self-referencing FK (WITH FK-ограничением); nullable; транспортная бронь обязана иметь тот же requestId** |
...
| OQ-DB-11 | **Скоуп транспортной брони**: тот же `requestId` или отдельный BookingRequest? | **CLOSED**: тот же `requestId`; транспортная техника обслуживает брони внутри той же заявки |
...
| # | Вопрос | Impact |
|---|--------|--------|
| OQ-DB-12 | **Атрибуция FO при запросе транспорта у коллег**: когда FO (ответственный за unwheeled-бронь) не берёт транспортную технику из своего флота, а запрашивает у другого FO — нужно ли фиксировать это в транспортной брони отдельным полем `transportRequestedFleetOwnerId`? Альтернатива: достаточно `assignedFleetOwnerId` на транспортной брони, если получившийся FO её принял. | Модель уведомлений / маршрутизация, v5 |
