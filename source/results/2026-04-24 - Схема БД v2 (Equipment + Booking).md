# Схема БД v2: Equipment + Booking (сводный файл)
...
| status | ENUM NOT NULL | Draft / Submitted / Confirmed / Declined / Revoked / Terminated / InProgress / Closed / EquipmentChanged / Extended |
...
| previousSnapshot | JSONB nullable | Снимок состояния Bookings до перехода. Заполняется при **EquipmentChanged** (`{"equipmentId": "...", "fleetId": "...", "assignedFleetOwnerId": "..."}`) и **Extended** (`{"endDt": "..."}`). При остальных переходах — NULL. |
...
| 2 | Bookings | Атомарная единица бронирования (одна техника) |
...
Отсутствие FK-ограничений — намеренное архитектурное решение: сервисы Users, Equipments и Fleets рассматриваются как внешние по отношению к Booking-блоку.
...
| # | Вопрос | Impact |
|---|--------|--------|
| OQ-DB-9 | **Unwheeled-флоу** (FR-NEW-29): три-шаговое согласование для негабаритной техники требует роли Transportation Responsible. Отдельная таблица `TransportRequests` или расширение Bookings? | Архитектура, v2 |
