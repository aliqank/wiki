...
**Errors:**
- `422 VALIDATION_ERROR` — `name` пустое или не уникальное
...
> Backend: копирует запись EquipmentTypes + все записи EquipmentTypeProperties (включая ссылки на те же Properties). `sortOrder` = MAX + 1. Frontend сразу редиректит на `GET /equipment-types/:newId` (карточку новой копии).
...
- `422` — `name` не уникально в Properties
...
|-------------|----------|
| `search` | Фильтр по `Properties.name` |
...
**Ключевые особенности API:**
- `GET /users?search=&userType=TCO|BP` — только чтение; создание/удаление недоступно
- `GET /users/:id` — для drawer; возвращает `externalId`, `departmentCode` и т.д.
...
*Документ подготовлен на основе ТЗ Admin Panel v2 и Схемы БД v5. Версия API: v1.*
