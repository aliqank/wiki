# Схема БД v10: Equipments + Bookings (сводный файл)
...
> **Ограничение booking workflow:** записи `Equipments` с `ownershipType = OnDemand` используются только для showcase/catalog и не могут создаваться в таблице `Bookings`.
...
> - LongTermRented: `Draft → Submitted → ConfirmedByFo → Confirmed → InProgress → Closed`
