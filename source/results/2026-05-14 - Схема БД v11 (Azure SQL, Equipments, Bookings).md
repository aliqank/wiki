...
- если в следующей итерации понадобится жёсткая DB-level проверка принадлежности fleet к конкретному BP, потребуется явная модель связи fleet/equipment с `BusinessPartners`.
