...
# Business Requirement Document — v10

**Version:** 10.0  
**Date:** 2026-04-22  
**Based on:** BRD v9 (2026-04-22) + Meeting: Open Questions Review 22.04.2026  
...
| # | Section | Change |
|---|---------|--------|
| 1 | 10. Open Questions | OQ-29 CLOSED: dynamic filter matrix — covered by dynamic characteristics admin tool |
| 2 | 10. Open Questions | OQ-36 CLOSED: aggregate request status — resolved by dev team |
| 3 | 10. Open Questions | OQ-NEW-A CLOSED: equipment field list per type — covered by dynamic characteristics admin tool |
| 4 | 10. Open Questions | OQ-NEW-B CLOSED: equipment/request template — dynamic characteristics pulled per equipment type automatically; no manual template selection |
| 5 | 10. Open Questions | OQ-NEW-C PARTIALLY CLOSED: SWP role defined — users with SWP role see JDE-sourced requests; users without role use Default WO; JDE trigger status still TBD |
| 6 | 10. Open Questions | OQ-NEW-E CLOSED: FO delegation via AAD groups (managed outside system); no in-system delegation feature required |
| 7 | 10. Open Questions | OQ-NEW-F CLOSED: equipment type parameters — covered by dynamic characteristics admin tool |
| 8 | 10. Open Questions | OQ-NEW-N CLOSED: equipment type fields and data examples — covered by dynamic characteristics admin tool |
| 9 | 10. Open Questions | OQ-NEW-P UPDATED: DataLake JDE jobs run once/day (~05:00 AM); direct JDE API integration proposed as preferred approach; pending JDE team |
| 10 | 10. Open Questions | OQ-NEW-Q PARTIALLY CLOSED: utilization updated once at end of working day; previous-day data shown in dashboard; satisfies Logistics; Maintenance team frequency TBD |
| 11 | 10. Open Questions | OQ-NEW-R added: whether long-term rented equipment transferred to another department requires additional CC approval for re-invoicing — to discuss at next BRD review |
| 12 | 3. Glossary | "Assigned" equipment clarified: bookable by Admin-authorized users with mandatory justification; Admin pre-authorization remains required; FO can decline |
| 13 | 3. Glossary | Service Work Processor clarified: role grants access to JDE-sourced requests; users without role work via Default WO only |
| 14 | 4.2 | Service Work Processor role definition updated (22.04) |
| 15 | 4.3 | FO item 11: delegation via AAD groups (outside system) — Confirmed/Closed; no in-system delegation feature |
| 16 | 4.5 | Admin item 13: pre-authorization for Assigned equipment — Confirmed; mandatory justification added as additional requirement |
| 17 | 4.5 | Admin item 14: delegation settings — Obsolete; delegation via AAD outside system |
| 18 | 5.1.1 | Equipment Type-Specific Parameters: approach confirmed — dynamic custom attributes per equipment type via Admin Panel tool |
| 19 | 6.1 | FR-NEW-02: Obsolete — FO delegation handled via AAD groups outside system |
| 20 | 6.2 | FR-010 updated: Admin manages custom characteristics per equipment type (dynamic attributes) — Confirmed |
| 21 | 6.2 | FR-NEW-04: Confirmed — Admin pre-authorization for Assigned equipment remains valid; mandatory justification added as additional requirement |
| 22 | 6.2 | FR-NEW-68 added: system shows only type-specific dynamic characteristics in request form and search filters |
| 23 | 6.3 | FR-NEW-08 updated: Assigned equipment bookable by Admin-authorized users with mandatory justification; FO can decline |
| 24 | 6.3 | FR-NEW-39: Confirmed — covered by dynamic characteristics admin tool |
| 25 | 6.12 | FR-NEW-62 updated: GIS tooltip to include current-day utilization (Maintenance requirement); refresh interval TBD (see OQ-NEW-Q) |
| 26 | 8.1 | JDE integration: DataLake jobs run once/day; direct JDE API proposed as preferred solution; pending JDE team |
| 27 | 8.5 | DataLake utilization: refresh once at end of working day; previous-day data displayed in dashboard |
| 28 | 8.5 | DataLake GIS: tracker types clarified (Teltonika — real-time; LoRaWAN — 3×/day; GPS-only sensors — 3×/day) |
...
| Area | Original BRD | Current Version (v10) |
|------|-------------|----------------------|
...
| GIS map | Basic coordinates display (FR-100) | Full GIS iframe integration (MAPH/Atlas team); speed/direction; historical route; tooltip with sensor data and current-day utilization |
| New roles | — | Transportation Responsible (for unwheeled equipment transport) |
| New concepts | — | Mobilization period; Default Work Order / Work Center; equipment freeze mechanism; Booking snapshot; dynamic characteristics per equipment type |
...
**Fleet Owner (FO)** – person responsible for approvals within a fleet. One fleet can have 2+ Fleet Owners (e.g. shift workers). A fleet has a shared team email used for system notifications. Delegation is managed via AAD group membership (outside the system).
...
- **Long-term rented** – rented long-term; treated as internal fleet for booking purposes. Managed by internal TCO Fleet Owner (not by BP). Booking process is identical to TCO Owned equipment.
- **On-demand rented (BP Showcase)** – rented on demand; displayed as catalog/showcase only in Phase 1.

**Dynamic Characteristics** *(Динамические характеристики)* – custom equipment attributes configured per equipment type via Admin Panel tool. These characteristics are shown automatically in the equipment card, request form, and search filters based on the selected equipment type. Replaces the static template approach. *(Added 22.04)*
...
- Utilization dashboard data is **refreshed once at the end of the working day**; dashboard shows data for the **previous day**. Satisfies Logistics requirements. Maintenance team refresh frequency — TBD *(OQ-NEW-Q — PARTIALLY CLOSED 22.04)*.
...
| FR-003 | Administrators, Service Work Processors defined by AAD groups | Pending (SWP trigger status — OQ-NEW-C) |
...
| FR-NEW-62 | GIS map marker **tooltip** on click: displays all available sensor data received from the last tracker synchronization, **plus current-day utilization** (Maintenance team requirement). Data fields: speed, heading, coordinates, engine hours, odometer, ignition, battery, current-day utilization value. Utilization refresh interval for map tooltip: TBD (target ~30–60 min; see OQ-NEW-Q). *(Updated 22.04 — original 20.04: all sensor data from last sync only)* | Pending alignment with GIS team + refresh interval confirmation |
...
| FR-NEW-64 | In the FO booking approval window: display an **equipment loading summary by dates** — a list of nearest active bookings for the given equipment unit. *(Added 20.04)* | Confirmed |
...
**Integration approach (Updated 22.04):**

> **Issue identified (22.04):** DataLake jobs for JDE run **once per day** (~05:00 AM). A Work Order step approved during the day would not appear in the booking tool until the following morning — unacceptable for operational use.

**Preferred solution (22.04):** Direct API integration with JDE E1 (bypassing DataLake for WO data). Allows near-real-time synchronization (target: ~5-minute interval).

**Action:** To confirm with the JDE team whether a 5-minute API interval is technically feasible. *(OQ-NEW-P — UPDATED)*

**Fallback:** If direct JDE API is not feasible, evaluate alternative approaches (e.g. more frequent DataLake jobs triggered by JDE events).

Integration path options:
- **Preferred:** JDE E1 → Direct API → HDV/HDE Booking Tool (target: ~5 min interval)
- **Fallback:** JDE E1 → Data Lake → HDV/HDE Booking Tool (current: once/day ~05:00)
...
> These fields are used to identify **repair days** in the utilization dashboard (FR-NEW-55). Repair days are excluded from average utilization calculation.

**Status:** Pending — JDE trigger status TBD; preferred integration path (direct API) pending JDE team confirmation.
...
**Utilization refresh frequency (22.04):**
- Utilization data is updated **once at the end of the working day**.
- Dashboard shows **previous day's** data.
- This frequency satisfies Logistics team requirements.
- Maintenance team frequency requirements — TBD *(OQ-NEW-Q — PARTIALLY CLOSED)*
...
Map visualization including **speed, heading, historical routes, sensor data, and current-day utilization** is sourced from the **Cosmos DB via API**.
...
| OQ-NEW-P | JDE update frequency: DataLake jobs run once/day (~05:00). Direct JDE API preferred (target ~5 min). Pending JDE team feasibility confirmation. | Ayan / JDE team | **UPDATED 22.04** — direct API proposed; TBD |
| OQ-NEW-Q | Utilization dashboard update frequency. Logistics: once at end of working day (previous day data) — **CLOSED for Logistics**. Maintenance team: requires clarification — possibly ~30–60 min for GIS map tooltip current-day utilization. | Team / Maintenance | **PARTIALLY CLOSED 22.04** — Logistics OK; Maintenance TBD |
| OQ-NEW-R | Long-term rented equipment transferred to another department: is additional CC Owner approval needed for re-invoicing? Previously confirmed: FO manages at own discretion, no additional approval. To re-confirm at next BRD review with new shift. | Team | **NEW — Open (22.04)** |
...
| FR-NEW-62 | GIS map tooltip — sensor data from last sync + current-day utilization (Maintenance requirement); refresh interval TBD | 17.04 / **Updated 22.04** | Pending (GIS team + interval) |
