...
# Business Requirement Document — v5

**Version:** 5.0  
**Date:** 2026-04-15  
**Based on:** BRD v4 (2026-04-13) + Requirements gathered in meeting 14.04 (~15:00)  
...
| # | Раздел | Изменение |
|---|--------|-----------|
| 1 | 3. Glossary | **Утилизация**: добавлены точные формулы расчёта (часы / км / оба типа); правило ограничения ≤ 100% |
| 2 | 4.2 Service Work Processor | Добавлен Action: организовать встречу с графистами (Асылбек, Диас) для подтверждения роли |
| 3 | 5.1 Equipment | Дизайн карточки: рабочие графики разделены на **день/ночь**; выходные дни учитываются: 06:00–06:00 (Long Term: 07:00–07:00) |
| 4 | 5.1 Equipment | Загрузка **нескольких фото** подтверждена отдельным требованием |
| 5 | 6.3 Equipment Management | FR-NEW-41 уточнён: из карточки техники можно **добавлять трекеры** через UI |
| 6 | 6.9 Status Management | **CONFLICT-02 RESOLVED**: FR-070, FR-071, FR-075 → ❌ Obsolete. Закрытие статуса — только вручную (R-75) |
| 7 | 6.9 Status Management | FR-074 уточнён: Request → **In Progress** при переходе **первого** бронирования (не считая отклонённых) в In Progress |
| 8 | 6.9 Status Management | Добавлен **FR-NEW-44**: Request → **Completed** при завершении **последнего активного** (не отклонённого) бронирования |
| 9 | 6.11 Reports | Добавлен **FR-NEW-45**: отдельная страница **«Завершённые заявки»** |
| 10 | 6.11 Reports | Добавлен **FR-NEW-46**: отчёты по утилизации (по дням, по отделам, по единицам техники) |
| 11 | 6.11 Reports | Добавлен **FR-NEW-47**: отчёт по **рабочим центрам** (work center — определение уточняется, OQ-NEW-G) |
| 12 | 8.1 JDE E1 | Добавлена таблица **требуемых полей Work Order** (WO number, name, status, priority, steps, step name) |
| 13 | 9. Conflicts | CONFLICT-02 → ✅ **RESOLVED** |
| 14 | 10. Open Questions | Добавлен **OQ-NEW-G**: что такое «рабочий центр» — атрибут техники или вид работ? |
| 15 | 10. Open Questions | Добавлен **OQ-NEW-H**: порог утилизации для подачи заявки — можно ли бронировать, если утилизация близка к 100%? |
...
>
> **🆕 [Updated 14.04] Required WO data fields** (confirmed by TCO):
...
| Priority | |
| Steps | Type of work (e.g. "Crane"); FO uses step to determine required equipment type |
| Step name | Need examples from TCO |
...
| 🆕 FR-NEW-47 | **[New 14.04]** System generates **work center reports** (отчёт по рабочим центрам); definition of "work center" pending clarification (OQ-NEW-G) | ⚠️ Open — pending OQ-NEW-G resolution |
...
**🆕 [Updated 14.04] Required Work Order fields from DataLake:**
...
| WO Priority | |
| Step (тип работ) | Defines required equipment type; e.g. "Crane"; FO uses step to identify what equipment is needed |
| Step Name | Examples needed from TCO |
...
| ~~OQ-NEW-D~~ | ~~Shared with Conditions vs. Shared without Conditions — is this distinction still meaningful?~~ | — | ✅ **CLOSED (13.04):** Difference = Justification field required. FO recall is independent mechanism. |
...
| **OQ-NEW-G** | **🆕 [New 14.04] "Work center" (рабочий центр)** — is this an attribute of equipment, or a type of work? Required to define report structure (FR-NEW-47) | TCO | ⚠️ Open — clarify at next meeting |
| **OQ-NEW-H** | **🆕 [New 14.04] Utilization booking threshold** — can a booking be submitted for equipment with utilization approaching 100%? If not, what is the cutoff threshold? | TCO | ⚠️ Open — clarify at next meeting |
...
| **FR-NEW-47** | **Work center reports (pending OQ-NEW-G definition)** | **14.04** | ⚠️ |
